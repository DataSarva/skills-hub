...
oc eval "(() => { $FIRE const c=[...document.querySelectorAll('button[aria-haspopup=menu]')].find(x=>/Video|Banana|Image/.test(x.innerText)); if(c)fire(c); return 'open'; })()" >/dev/null 2>&1
sleep 1
oc eval "(() => { $FIRE const m=document.querySelector('[role=menu]')||document.querySelector('[data-radix-menu-content]'); if(!m)return 'no panel'; const els=[...m.querySelectorAll('button,[role=menuitem],[role=menuitemradio],[role=radio]')]; const pick=t=>{const e=els.find(x=>(x.innerText||'').trim()===t); if(e){fire(e);return true} return false}; const vid=els.find(x=>{const t=(x.innerText||'').trim(); return t.includes('Video')&&t.length<16}); if(vid)fire(vid); pick('9:16'); pick('${DUR}s'); pick('1x'); return 'set'; })()" >/dev/null 2>&1
