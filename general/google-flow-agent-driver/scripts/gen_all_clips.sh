...
LEAD="Cinematic devotional scene. "   # ensures prompt never starts with '@' (Slate mention-mode guard)
...
for ID in $IDS; do
  ...
  DUR=$(python3 -c "import json;print([s['dur'] for s in json.load(open('$ROOT/shots.json'))['shots'] if s['id']==$ID][0])")
